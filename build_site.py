...
def og_image(slug: str) -> str:
    """Absolute URL of the self-hosted 1200x630 share card (static/og/<slug>.jpg,
    copied to site/og/ at build; generated by make_og.py). Empty if none exists.
    Self-hosting keeps cards small (the raw art CDN serves >5 MB PNGs that
    Twitter/Facebook reject) and avoids a runtime dependency on the resizer."""
    return f"{SITE_URL}og/{slug}.jpg" if (STATIC / "og" / f"{slug}.jpg").exists() else ""
